...
function enforcePolicy(context) {
  ...
  // Check 1: Max rooms per student
  if (context.existingApproved >= POLICY.maxRoomsPerStudent) {
    violations.push(`POLICY:MAX_ROOMS — Student already holds ${context.existingApproved} active allocation(s). Maximum allowed: ${POLICY.maxRoomsPerStudent}.`);
  }

  // Check 2: Price cap — check actual assigned price OR student's preferred price
  const priceToCheck = context.price > 0 ? context.price : (context.preferredPrice || 0);
  const typeToCheck  = context.roomType || context.preferredType;
  if (typeToCheck && priceToCheck > 0) {
    const cap = POLICY.priceCapPerType[typeToCheck];
    if (cap && priceToCheck > cap) {
      violations.push(`POLICY:PRICE_CAP — Selected ${typeToCheck} room at ₹${priceToCheck.toLocaleString('en-IN')}/mo exceeds policy cap of ₹${cap.toLocaleString('en-IN')}/mo for ${typeToCheck} type. Application flagged.`);
    }
  }

  // Check 3: Price cap info — type known but no price (truly unknown price scenario)
  if (typeToCheck && priceToCheck === 0) {
    const cap = POLICY.priceCapPerType[typeToCheck];
    if (cap) {
      warnings.push(`POLICY:PRICE_CAP_INFO — Preferred type "${typeToCheck}" has a policy cap of ₹${cap.toLocaleString('en-IN')}/mo. Admin must assign a compliant room.`);
    }
  }

  // Check 4: Room availability — only flag if a room is actually assigned and not available
  if (context.roomStatus && context.roomStatus !== "Available" && context.roomStatus !== "Unknown" && context.roomStatus !== null) {
    violations.push(`POLICY:AVAILABILITY — Room is currently ${context.roomStatus}. Cannot allocate an unavailable room.`);
  }

  // Check 5: SLA — application pending too long
  if (context.pendingDays && context.pendingDays > POLICY.maxPendingDaysBeforeAlert) {
    warnings.push(`POLICY:SLA_WARNING — Application pending for ${context.pendingDays} days, exceeds ${POLICY.maxPendingDaysBeforeAlert}-day SLA. Needs immediate review.`);
  }
...
router.get("/compliance-agent/scan", async (req, res) => {
  try {
    ...
    for (const app of apps) {
      ...
      // Only look up room if one is actually assigned
      const room = app.roomNumber ? await Room.findOne({ number: app.roomNumber }) : null;

      const existingApproved = await Application.countDocuments({
      ...
      const policy = enforcePolicy({
        existingApproved,
        // Use room type: actual assigned room > preferredType from application
        roomType: app.roomNumber
          ? (room ? room.type : null)
          : (app.preferredType || null),
        // Use roomPrice if set (student chose a room with a price), otherwise 0
        price: app.roomPrice && app.roomPrice > 0 ? app.roomPrice : 0,
        // Only pass room status if a real room is assigned
        roomStatus: room ? room.status : null,
        pendingDays,
        // Pass preferred price cap check — if student selected a room type+price, validate it
        preferredPrice: app.roomPrice && app.roomPrice > 0 ? app.roomPrice : 0,
        preferredType: app.preferredType || null,
      });

      if (pendingDays > POLICY.maxPendingDaysBeforeAlert) slaBreaches++;
      totalViolations += policy.violations.length;

      // Always include in report if there's anything to flag
      if (!policy.compliant || policy.warnings.length || pendingDays > POLICY.maxPendingDaysBeforeAlert) {
        const entry = await audit(
          ...
          app.roomNumber || null,
          policy.compliant ? "WARNING" : "FLAG",
          `Scanned application for ${app.studentName}. Preferred type: ${app.preferredType || "Any"}. Pending ${pendingDays} day(s).`,
          policy.violations,
          policy.warnings
        );
        report.push({
          application: app,
          pendingDays,
          preferredType: app.preferredType || "Any",
          preferredBuilding: app.preferredBuilding || "Any",
          violations: policy.violations,
          warnings: policy.warnings,
          auditEntry: entry,
        });
    ...
    const summaryEntry = await audit(
      "ComplianceAgent", "scan-summary", null, null,
      `SCANNED ${apps.length} applications`,
      `Found ${totalViolations} violations, ${slaBreaches} SLA breaches across ${apps.length} pending applications.`
    );
  ...
  try {
    ...
    for (const app of apps) {
      ...
      if (missingPayments > 0 || !paidMonths.includes(currentMonth)) {
        ...
        const sev = missingPayments >= 3 ? "critical" : missingPayments >= 1 ? "warning" : "info";
        const entry = await audit(
          "FeeAgent", "overdue-scan",
          app.studentId, app.roomNumber,
          sev === "critical" ? "ESCALATE" : "ALERT",
    ...
    const totalOutstanding = alerts.reduce((s, a) => s + a.outstanding, 0);
    const summaryEntry = await audit(
      "FeeAgent", "scan-complete", null, null,
      `SCANNED ${apps.length} allocations`,
...
router.get("/complaint-agent/triage", async (req, res) => {
  try {
    ...
    for (const c of complaints) {
      ...
      const decision = slaBreach || playbook.autoEscalate ? "ESCALATE" : "MONITOR";
      const violations = slaBreach
        ? [`SLA breach: ${c.priority} complaint open for ${ageHours}h (SLA: ${playbook.sla})`]
        : [];

      const entry = await audit(
        "ComplaintAgent", "triage",
        c.studentId, c.room, decision,
        `Complaint: "${c.subject}" | Priority: ${c.priority} | Age: ${ageHours}h | SLA: ${playbook.sla}`,
        violations
      );
    ...
    const escalations = triage.filter(t => t.decision === "ESCALATE").length;
    const summaryEntry = await audit(
      "ComplaintAgent", "triage-complete", null, null,
      `TRIAGED ${complaints.length} complaints`,
      `${escalations} require immediate escalation. ${triage.filter(t => t.slaBreach).length} SLA breaches detected.`
    );
...
router.get("/occupancy-agent/report", async (req, res) => {
  try {
    const [rooms, apps] = await Promise.all([
      Room.find({}),
    ...
    for (const room of rooms.filter(r => r.status === "Occupied")) {
      ...
      if (!hasApp) {
        const entry = await audit(
          "OccupancyAgent", "anomaly-detect", null, room.number, "FLAG",
          `Room ${room.number} (Building ${room.building}) marked Occupied but has no approved application.`,
          [`DATA_ANOMALY: Room ${room.number} occupied without matching allocation`]
        );
        anomalies.push({ room, auditEntry: entry });
    ...
    for (const app of apps) {
      ...
      if (room && room.status !== "Occupied") {
        const entry = await audit(
          "OccupancyAgent", "mismatch-detect", app.studentId, app.roomNumber, "FLAG",
          `Approved application for Room ${app.roomNumber} but room status is ${room.status}.`,
          [`DATA_MISMATCH: Approved app but room not marked Occupied`]
        );
        allocationMismatches.push({ application: app, room, auditEntry: entry });
    ...
    const healthStatus = occupancyRate > 90 ? "critical" : occupancyRate > 75 ? "warning" : "healthy";
    const summaryEntry = await audit(
      "OccupancyAgent", "report", null, null,
      `OCCUPANCY ${occupancyRate}%`,
      `Total: ${total} rooms | Occupied: ${occupied} | Available: ${available} | Anomalies: ${anomalies.length} | Status: ${healthStatus}`
    );
...
router.post("/run-all", async (req, res) => {
  try {
    ...
    await audit(
      "MasterOrchestrator", "full-scan-complete", null, null,
      totalIssues === 0 ? "ALL_CLEAR" : `${totalIssues} ISSUES FOUND`,
      `Compliance: ${summary.complianceFlagged} flags | Fees: ${summary.feeAlerts} alerts | Complaints: ${summary.complaintEscalations} escalations | Occupancy: ${summary.occupancyAnomalies} anomalies`
    );
...
router.post("/demo/double-allocation", async (req, res) => {
  try {
    ...
    const auditEntry = await audit(
      "AllocationAgent", "double-allocation-check",
      studentId, null, decision, reasoning,
...
router.post("/demo/occupied-room-approval", async (req, res) => {
  try {
    ...
    const auditEntry = await audit(
      "ComplianceAgent", "room-availability-check",
      null, roomNumber, decision, reasoning,
...
router.post("/demo/sla-breach-escalation", async (req, res) => {
  try {
    ...
    for (const c of breached) {
      ...
      const auditEntry = await audit(
        "ComplaintAgent", "sla-auto-escalate",
        c.studentId, c.room, "ESCALATED",
